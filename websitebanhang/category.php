...
if (isset($_SESSION['searchCate'])){
    $search = $_SESSION['searchCate'];
    $where = "AND `sanpham`.`TenSP` LIKE '%$search%'";
}

$item = mysqli_query($link,"SELECT *,`Ten` FROM `sanpham`, `loaisanpham` WHERE `sanpham`.`MaLSP` = $id AND `sanpham`.`MaLSP` = `loaisanpham`.`MaLSP` " . $where);
                ...
                <div class="col-md-6">
                    <div class="header-search">
                        <form  method="post">
                            ...
                            <input class="input" name="searchCate" placeholder="Search here">
